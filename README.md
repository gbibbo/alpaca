...
Python trading infrastructure prototype for paper trading and reproducible backtesting with Alpaca. It implements the path from market data to strategy signals, risk-gated order intents, broker execution, persisted fills, API access, and monitoring. The focus is the trading system around the signal: event transport, typed contracts, risk controls, broker idempotency, observability, persistence, and repeatable simulations.
...
**Built:** a service-oriented trading workflow with separate components for data ingestion, strategy generation, risk validation, order execution, historical simulation, PnL aggregation, API access, and monitoring.

**Stack:** Python, FastAPI, Redis Streams, Alpaca API, Pydantic v2, Prometheus, Grafana, WebSockets, SQLite, CSV/Parquet export, pytest, Docker Compose, GitHub Actions.

**Code paths worth opening first:**

- `apps/risk_manager/main.py`: validates signals before execution using market hours, confidence, expiry, sizing, limits, rate limiting, circuit breakers, and deduplication.
- `apps/executor/main.py`: submits Alpaca paper orders and tracks the order lifecycle with retry and duplicate-order safeguards.
- `apps/simulator/main.py` and `scripts/sim_random.py`: replay historical data and generate reproducible backtest outputs.
- `apps/api/main.py`: exposes health, portfolio, signals, backtest jobs, WebSocket dashboard, and Prometheus metrics.
- `lib/models.py`, `lib/bus_streams.py`, `lib/order_fsm.py`, `lib/deduplication.py`: shared contracts and infrastructure used across services.
- `tests/`, `scripts/qa/`, `.github/workflows/`: unit, integration, load, stream, metrics, end to end, CI, nightly, and release validation.

**Implemented behavior:** Redis Streams consumer groups, typed event contracts, deterministic client order IDs, duplicate order blocking, NYSE/NASDAQ market-hours checks, stale signal rejection, rate limiting, broker retry handling, partial fills, portfolio/PnL aggregation, WebSocket updates, Prometheus metrics, persisted backtest output, and fill-hash reproducibility.

## Example generated artifact

![GOOGL backtest results](out/GOOGL.png)

The included GOOGL plot is generated by `scripts/sim_random.py`. It shows portfolio value, initial capital, and drawdown over time for a reproducible random 50/50 strategy run. The strategy is intentionally simple; the artifact exists to verify that the data, simulation, accounting, plotting, and export path works end to end.
