...
"""
    ...
    def kill_processes_by_port(self, port: int, service_name: str = "unknown"):
        """Kill all processes using a specific port - ENHANCED VERSION"""
        killed_pids = []
        
        try:
            # Method 1: Use psutil to find processes by port
                    ...
                    if connections:
                        for conn in connections:
                            if hasattr(conn, 'laddr') and conn.laddr and conn.laddr.port == port:
                                ...
                                print(f"🔫 Killing process {pid} ({proc_name}) using port {port}")
            ...
            # Method 2: Try lsof as backup (if available in environment)
            try:
                    ...
                    for pid_str in pids:
                        if pid_str and pid_str.isdigit():
                            ...
                            if pid not in killed_pids:
                                print(f"🔫 Killing additional process {pid} found by lsof on port {port}")
                                try:
                                    os.kill(pid, signal.SIGTERM)
                                    time.sleep(1)
                                    if psutil.pid_exists(pid):
    ...
    def start_api(self):
        """Start API server - ENHANCED WITH STRICT HEALTH CHECKING"""
        ...
        cmd = [
            ...
            "--host", "0.0.0.0",
            "--port", "8000",
            "--no-reload",  # CRITICAL: No reload to avoid child processes
            "--log-level", "info"
        ]
        ...
        if self.start_service_quiet("api", cmd, env=env):
            # FIFTH: Strict health check with NO FALSE POSITIVES
            print("⏳ Performing strict health check...")
            health_passed = False
            
            for attempt in range(1, 16):  # 15 attempts
                try:
                    print(f"  🩺 Health check attempt {attempt}/15...")
            ...
            # SIXTH: STRICT FAILURE HANDLING - NO FALSE POSITIVES
    ...
    def start_prometheus(self):
        ...
        if self.start_service_quiet("prometheus", cmd, cwd=str(self.prometheus_dir)):
            ...
            try:
                ...
                print("⚠️ Prometheus started but not responding")
            return True
        return False
    ...
    def start_trading_service(self, service_name):
        """Start a single trading service"""
