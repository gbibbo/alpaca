...
"""
    ...
    def kill_processes_by_port(self, port: int, service_name: str = "unknown"):
        """Kill all processes using a specific port - FIXED VERSION"""
        killed_pids = []
        
        try:
            # Method 1: Use psutil with safer connection checking
            for proc in psutil.process_iter(['pid', 'name']):
                try:
                    # Get the actual process object to check connections
                    process = psutil.Process(proc.info['pid'])
                    
                    # Try to get connections - this may fail on some systems
                    try:
                        ...
                        # Can't access connections for this process, skip
                        pass
