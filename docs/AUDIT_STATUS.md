...
- **done** — Operational sizing parity: `apps/risk_manager` now sizes to the allocation cap when `stop_loss_pct` is disabled (Settings accept `None` for stop/take-profit) instead of dividing by the stop. The live executor still *requires* protective exits on BUY as a safety default; lifting that for pure trend strategies in paper is a deliberate, separate change.
