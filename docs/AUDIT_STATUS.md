...
- **done** — Durable-operation guard: at service startup (`connect_bus`), `trading_mode` paper/live refuses to run on fakeredis or a Pub/Sub bus (fire-and-forget); Streams on real Redis is required. Enforced at startup (not on bus construction, so tooling/tests can still build a bus); backtest is unaffected. Tested (`tests/test_durable_bus_guard.py`).
